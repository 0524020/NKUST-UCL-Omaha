...
_ONECLICK_PLUGIN_NAME = 'npUCLabOneClick'
_UPDATE_PLUGIN_NAME = 'npUCLabUpdate'
_CRASH_HANDLER_NAME = 'UCLabCrashHandler'
...
def _GetMetainstallerPayloadFilenames(prefix,
  ...
  payload_files = [
      ...
      plugin_dll_name,
      'UCLabUpdateHelper.msi',
      'UCLabUpdateBroker.exe',
      'UCLabUpdateOnDemand.exe',
      'UCLabUpdateComRegisterShell64.exe',
      'UCLabUpdateWebPlugin.exe',
  ...
  if _IsSupportedOmaha2Version(omaha_version):
    ...
    payload_files.remove('UCLabUpdateBroker.exe')
    payload_files.remove('UCLabUpdateOnDemand.exe')
    payload_files.remove('UCLabUpdateComRegisterShell64.exe')
  ...
  if (omaha_version[0] >= 1 and
    ...
    payload_files.append('UCLabUpdateCore.exe')
